...
const logStates =[ 'error', 'warn', 'info', 'debug'];
function logError (message){ if (logStates[0].indexOf (logState) >=0) console.log (message); }
function logWarn (message){ if (logStates.splice (0, 2).indexOf (logState) >=0) console.log (message); }
function logInfo (message){ if (logStates.splice (0, 3).indexOf (logState) >=0) console.log (message); }
function logDebug (message){ if (logStates.indexOf (logState) >=0) console.log (message); }
// affichage de base
var debbyPlay ={
...
useTsv = function (tsvFile){
	var xhttp = new XMLHttpRequest();
	xhttp.open ('GET', tsvFile, false);
	xhttp.send();
	var res =[];
	if (xhttp.status ==200){
		var resTmp = xhttp.responseText.clean();
		res = resTmp.split ('\n');
		for (var l=0; l< res.length; l++) res[l] = res[l].split ('\t');
	}
	return res;
}
